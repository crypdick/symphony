defmodule SymphonyElixir.Codex.DynamicTool do
  ...
  defp normalize_sync_workpad_args(%{} = args) do
    with {:ok, issue_id} <- required_sync_workpad_arg(args, :issue_id, "`issue_id` is required"),
         {:ok, file_path} <- required_sync_workpad_arg(args, :file_path, "`file_path` is required") do
  ...
  defp required_sync_workpad_arg(args, key, message) do
    case sync_workpad_arg(args, key) do
      value when is_binary(value) and value != "" -> {:ok, value}
      _ -> {:error, {:sync_workpad, message}}
    end
  end
  ...
  defp sync_workpad_arg(args, key) when is_atom(key) do
